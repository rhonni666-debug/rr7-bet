...
export function GamePage() {
  ...
  const { balance, playDemo } = useDemo();
  const { user } = useAuth();
  ...
  const provider = providers.find((item) => item.id === currentGame.providerId);
  ...
  async function runDemo() {
    ...
    setBusy(true);
    const outcome = await playDemo(currentGame.slug, currentGame.name, bet);
    setBusy(false);
