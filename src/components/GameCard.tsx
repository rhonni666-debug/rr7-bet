...
import type { DemoGame, Provider } from '../types';

export function GameCard({ game, provider }: { game: DemoGame; provider?: Provider }) {
  ...
  return (
    <article className="group min-w-0">
      <div className={`relative aspect-[4/5] overflow-hidden rounded-2xl bg-gradient-to-br ${game.accent} shadow-lg shadow-black/20`}>
        {game.thumbnailUrl ? <img src={game.thumbnailUrl} alt="" className="absolute inset-0 h-full w-full object-cover" loading="lazy" /> : null}
        ...
        {!game.thumbnailUrl && <div className="absolute inset-0 grid place-items-center pb-8 text-6xl drop-shadow-2xl transition-transform duration-300 group-hover:scale-110">{game.art}</div>}
